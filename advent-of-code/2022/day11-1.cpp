...
struct monkey_t {
    ...
    monkey_t(const std::vector<std::string> &group) {
        ...
        if (!strcmp(op_buf, "+")) op = std::plus<int>();
        if (!strcmp(op_buf, "*")) op = std::multiplies<int>();
...
int solve(std::vector<std::unique_ptr<monkey_t>> &monkeys) {
    ...
    std::vector<int> all_items_inspected;
    for (auto &&monkey_ptr : monkeys) {
